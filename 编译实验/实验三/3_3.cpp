...
stack<int>term;
stack<int>factor;
stack<string>condition_truelist;
stack<string>condition_falselist;
stack<string>statement_nextlist;
stack<string>statement_list_nextlist;
...
void backpatch(string truelist,int M)
{
	...
	{
		int p = (int)truelist[i];
		char *str = work3_p[p] + result;
		insert_num(str+1,q);
	}
}
...
void work3_trainslate(int state,int k)
{
	...
	else if(state >= 23 && state <= 28)
	{
		int re = G[state][1];
		rela.push(re);
	}
	else if(state == 21) // E1 rep E2
	{
		string truelist = "";
		truelist += p_len;
		...
		condition_truelist.push(truelist);
		condition_falselist.push(falselist);
		...
		//op
		str[op] = 'J';
		int len = strlen(String[rel]);
		for(int i = 0; i < len; ++i) str[op+i+1] = String[rel][i];
	...
	//	return;
		...
		str[op] = 'J';
		str[result] = 'L';
	//	insert_num(str+result+1,p_len);
		p_len++;
		
	}
	else if(state == 41) //M.instr = nextlist
	{
		M.push(p_len);
	}
	else if(state == 16)//statement   = if  condition then M statement1 M $
	{
		string truelist = condition_truelist.top();condition_truelist.pop();
		string falselist = condition_falselist.top();condition_falselist.pop();	
		int M_list1 = M.top();M.pop();
		int M_list2 = M.top();M.pop();
		
		
		backpatch(truelist,M_list2);
		
		backpatch(falselist,M_list1);
		
		string s_nextlist1 = statement_nextlist.top();statement_nextlist.pop();	
		string s_nextlist = falselist+s_nextlist1;//merge	
		statement_nextlist.push(s_nextlist);
	...
	{
		string statement_list  = statement_list_nextlist.top();statement_list_nextlist.pop();
		statement_nextlist.push(statement_list);
	}
	else if(state == 19 || state == 20)
	{
		string list  = statement_nextlist.top();statement_nextlist.pop();
		statement_list_nextlist.push(list);
	}
	else if(state == 18)
	{
		string ss = "";
		int a = 0;
		ss += a;
		statement_nextlist.push(ss);
	}
	else if(state == 17) //statement   =   while M1 condition M2 do statement1 M3 $
	{
		string statement1 = statement_nextlist.top();statement_nextlist.pop();
		string ctruelist = condition_truelist.top();condition_truelist.pop();
		string cfalselist = condition_falselist.top();condition_falselist.pop();
		
		int M3 = M.top();M.pop();
		int M2 = M.top();M.pop();
		int M1 = M.top();M.pop();
		backpatch(statement1,M1);
		backpatch(ctruelist,M2);;
		backpatch(cfalselist,M3+1);
		
		statement_nextlist.push(cfalselist);
		
		cout << "M1 = " << M1 << endl;
		char *str = work3_p[p_len];
		str[op] = 'J';
		str[result] = 'L';
		insert_num(str+result+1,M1);
		p_len++;
	}
}
...
{		
	...
	char str[] = " L        op          arg1          arg2          result";
	fprintf(fp_work3,"%s\n",str);
...
void work3_Print()
{
	for(int i = 1; i < p_len; ++i) fprintf(fp_work3,"%s\n",work3_p[i]);
}
...
void work3_init()
{
	...
	memset(read_op,0,sizeof(read_op));
	read_len = 0;
	memset(work3_p,' ',sizeof(work3_p));
	addr = p_len = 1;
	for(int put_i = 1; put_i <= 250; ++put_i)
	{
	
		char *str= work3_p[put_i];
		str[0] = '(';
		str[70] = '\0';
		int op = 10,arg1 = 22,arg2 = 35,result = 50;
		int num = put_i;
